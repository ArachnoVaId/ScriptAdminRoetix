// IMOTION Marketing Plan Competition - Extend Phase & Add 30 Fields
...
var FIELDS = [
  ...
  { key: 'LeaderInstitution_extend', label: 'Team Leader Institution (First Member)', type: 'text', required: true,
    notes: 'Contoh: Universitas Indonesia' },
  { key: 'LeaderMajorBatch_extend', label: 'Team Leader Major and Batch (First Member)', type: 'text', required: true,
    notes: 'Contoh: Manajemen 2025' },
  { key: 'LeaderEmail_extend', label: 'Email Ketua Tim (Anggota Pertama)', type: 'email', required: true },
  { key: 'LeaderPhoneNumber_extend', label: 'Team Leader Whatsapp Number (First Member)', type: 'phone', required: true },
  { key: 'LeaderLineID_extend', label: 'Team Leader LINE ID (First Member)', type: 'text', required: true },
  // Second Member (Anggota Kedua)
  { key: 'Member1FullName_extend', label: 'Nama Anggota Kedua', type: 'text', required: true },
  { key: 'Member1Institution_extend', label: 'Second Member Institution', type: 'text', required: true,
    notes: 'Contoh: Universitas Indonesia' },
  { key: 'Member1MajorBatch_extend', label: 'Second Member Major and Batch', type: 'text', required: true,
    notes: 'Contoh: Manajemen 2025' },
  ...
  { key: 'Member1LineID_extend', label: 'Second Member LINE ID', type: 'text', required: true },
  // Third Member (Anggota Ketiga)
  { key: 'Member2FullName_extend', label: 'Nama Anggota Ketiga', type: 'text', required: true },
  { key: 'Member2Institution_extend', label: 'Third Member Institution', type: 'text', required: true,
    notes: 'Contoh: Universitas Indonesia' },
  { key: 'Member2MajorBatch_extend', label: 'Third Member Major and Batch', type: 'text', required: true,
    notes: 'Contoh: Manajemen 2025' },
  ...
  { key: 'Member2LineID_extend', label: 'Third Member LINE ID', type: 'text', required: true },
  // Fourth Member (Anggota Keempat — Optional)
  { key: 'Member3FullName_extend', label: 'Nama Anggota Keempat (opsional)', type: 'text', required: false },
  { key: 'Member3Institution_extend', label: 'Fourth Member Institution (opsional)', type: 'text', required: false,
    notes: 'Contoh: Universitas Indonesia' },
  { key: 'Member3MajorBatch_extend', label: 'Fourth Member Major and Batch (opsional)', type: 'text', required: false,
    notes: 'Contoh: Manajemen 2025' },
  { key: 'Member3Email_extend', label: 'Email Anggota Keempat (opsional)', type: 'email', required: false },
  { key: 'Member3PhoneNumber_extend', label: 'Nomor WhatsApp Anggota Keempat (opsional)', type: 'phone', required: false },
  { key: 'Member3LineID_extend', label: 'Fourth Member LINE ID (opsional)', type: 'text', required: false },
  // Attachments
  ...
  { key: 'OutsideJabodetabek_extend', label: 'Apakah domisili tim anda atau salah satu anggota dari tim anda berasal dari LUAR JABODETABEK?', type: 'multiple_choice', required: true,
    options: ['Ya/Mau Mendaftar', 'Tidak'] },
  { key: 'SupportPackageProof_extend', label: 'Pengumpulan Bukti Formulir IMOTION Support Package', type: 'file', required: true,
...
(function run() {
  console.log('%c═══ Marketing Plan Competition - Extend Phase + ' + FIELDS.length + ' Fields ═══', 'color:#6366f1;font-weight:bold');
  console.log('Phase index: ' + PHASE_INDEX);

  fill(byName('timelines.' + PHASE_INDEX + '.name'), 'Marketing Plan Competition Extend Registration');
  console.log('Phase Name -> Marketing Plan Competition Extend Registration');
