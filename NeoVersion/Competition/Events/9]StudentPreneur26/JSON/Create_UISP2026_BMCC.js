// NEW ADMIN UI (admin.roetix.com/events/create atau /events/UISP2026BMCC/edit) - The 16th UI Studentpreneurs
// - BMCC Registration - subevent EARLY BIRD (1 dari 3: Early Bird / Normal Price / Late Bird)
// Companion doc: ../../NewAdminUI/README.md (DOM notes umum) + ./RECAP_UISP2026BMCC_live.md (recap field
// live per 2026-08-25, dipakai sebagai sumber data script ini, BUKAN RAW_Studentpreneur.txt yang sudah usang)
// File kembar: Create_UISP2026_BMCC_NormalPrice.js, Create_UISP2026_BMCC_LateBird.js (isi identik,
// beda IDENTITY/PHASE1/TIME_PRICE_CELLS saja - lihat komentar "BEDA ANTAR SUBEVENT" di tiap file)
//
// KENAPA 3 SUBEVENT TERPISAH (bukan 1 event 3 Timeline seperti versi lama script ini):
...
//   bawah) supaya bisa verifikasi visual di layar antara Phase 1 selesai dan Phase 2 mulai diisi, sebelum
//   makin banyak yang harus diulang kalau ternyata section Phase 1  galah ke-mount bareng Phase 2.
//
// STRUKTUR:
...
//
// KEPUTUSAN atas ambiguitas data (dikonfirmasi user 2026-08-25, lihat RECAP_UISP2026BMCC_live.md utk detail):
//   1. Field 'Team Composition' di live cuma py 2 opsi (Individual, 2 Members) - opsi '3 Members' HILANG,
//      padahal deskripsi bilang tim 1-3 orang. Diperbaiki di sini jadi 3 opsi.
//   2. Field 'How did you know...' di live cuma 5 opsi - opsi 'Other' HILANG. Diperbaiki jadi 6 opsi.
//   3. Field 'Are you interested to join...' di live cuma 1 opsi (efektif rusak) - opsi 'No, I'm only
//      interested of this competition.' HILANG. Diperbaiki jadi 2 opsi.
//   4. Section 'Registration requirements': live sudah dikonsolidasi dari 6 field upload terpisah jadi 1
//      field upload gabungan ('Attach the combined files here', .pdf) + checklist di description - dipakai
//      apa adanya (keputusan sebelumnya, bukan bug), TAPI baris '(Proof) Follow Instagram Our Assessor
//      Partner' dirapikan: placeholder '(....)' & baris duplikat '*@bvi.id' di live digabung jadi 1 baris
//      bersih '@bvi.id'.
//   5. Typo 'Share Poster the 15th UI Studentpreneurs' -> diperbaiki jadi '16th' (dikonfirmasi user).
//   6. Tanggal Phase/Timeline 'Early Bird' di live event yang SEKARANG tertulis Agustus (08/19, 08/23) -
//      dikonfirmasi user itu SENGAJA untuk testing form, BUKAN tanggal final. Script ini pakai tanggal ASLI
//      dari tabel gelombang (September, lihat PHASE1 di bawah) untuk hasil akhir yang sebenarnya dipakai.
//   7. Section description Member 1/2 pakai teks live 'Skip the section if you are registering as an
//      individual' (beda dari field-level note 'Mark "-" if you are registering as an individual' - dua
//      teks ini memang beda & keduanya dipertahankan sesuai live, bukan salah ketik).
//
// STATES (Step 2) - diisi eksplisit sesuai live (BUKAN default platform, live event override manual):
//   Active ON, Featured OFF, Team size ON (Min 1 / Max 1), Referral codes ON, Detailed pricing ON.
//
// EVENT INI (Early Bird) MEMAKAI ULANG Event ID 'UISP2026BMCC' yang SUDAH ADA (live, isinya belum lengkap -
// lihat RECAP). Sebelum paste script ini: buka event itu, jalankan Reset_UISP2026_BMCC.js dulu (clear draft
// + reload) baru paste script ini dari draft kosong. JANGAN paste ke event lain / event ID baru.
//
...
//     dikirim user) - script ini asumsikan SEMUA 5 field Phase 2 required (masuk akal utk form re-registrasi
//     + bukti bayar). Koreksi manual di Step 4 Phase 2 kalau ada yang harusnya optional.
//   - Format file diterima Phase 2 (pdf/image) - live baru terverifikasi field upload single-format '.pdf'
//     (lihat README). Kalau UI Answer Type 'file' tidak punya opsi multi-format, upload proof mungkin
//     WAJIB pdf-only meski deskripsi tulis "pdf/image" - PERIKSA MANUAL saat isi Phase 2 field ke-5.
...
var IDENTITY = {
  eventName: 'The 16th UI Studentpreneurs - Early Bird BMCC Registration',
  eventId: 'UISP2026BMCC', // sudah ada (live) - JANGAN diubah, event ID immutable setelah dibuat
  organizerName: 'BEM FEB UI',
  description: EVENT_DESCRIPTION
};

// Phase 1 - jendela gelombang Early Bird sesuai tabel (19-29 September 2026), GRATIS.
var PHASE1 = { name: 'Registrasi Preliminary Early Bird', start: '2026-09-19T00:00', end: '2026-09-29T23:59' };
...
var TIME_PRICE_CELLS = [
  ...
  { label: PHASE2.name, price: 185000, feeType: 'flat', fee: 11000, taxType: 'flat', tax: 0 }
];
...
async function addSectionsAndFields(sections, expectedBaseline) {
  // Guard DIPERKETAT dari versi lama: sebelumnya cuma cek "harus 0". Sekarang terima expectedBaseline
  // karena Phase 1 (5 section) mungkin masih ke-mount bareng Phase 2 kalau ternyata UI tidak accordion
  // per-Phase (BELUM DIVERIFIKASI - lihat peringatan di header file). Kalau count tidak sama persis dengan
  // yang diharapkan, STOP total - jangan coba lanjut nebak index, resiko field nyasar ke Phase yang salah.
  ...
  if (existingSectionTitles.length !== expectedBaseline) {
    console.error('%cSTOP: ada ' + existingSectionTitles.length + ' section title input di DOM sekarang, diharapkan tepat ' + expectedBaseline + '. Kemungkinan (a) draft lama belum di-reset -> jalankan Reset_UISP2026_BMCC.js dulu, ATAU (b) section Phase lain masih ke-mount bareng Phase yang sedang dikerjakan (kalau begitu fungsi generic ini TIDAK AMAN dipakai apa adanya - perlu scoping manual per-Phase, belum didukung script ini). PERIKSA LAYAR sebelum lanjut apa pun.', 'color:#ef4444;font-weight:bold;font-size:13px');
