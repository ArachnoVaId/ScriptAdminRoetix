// NEW ADMIN UI (admin.roetix.com/events/create) - BNAT XV 2026 Talkshow - Beyond Numbers - Peserta Lomba & Guru Pendamping
...
//
// HARGA: price 0, serviceFee flat 0, tax 0.
//   Peserta membayar Rp0. Cocokkan angka ini di layar Step 4 > Time-Price.
//
// VARIAN GRATIS - KHUSUS PESERTA LOMBA & GURU PENDAMPING. Beda dari versi berbayar:
//   1. HARGA Rp0 (price 0, fee 0). Peserta lomba YAC/ACC dan Guru Pendamping YAC tidak
//      membayar tiket talkshow - kehadiran mereka bagian dari catatan kehadiran resmi tim,
//      bukan pembelian tiket. Peserta Umum tetap membayar lewat event talkshow berbayar.
//   2. CABANG PESERTA UMUM DIHAPUS: section "Informasi Peserta Umum" (berisi Kategori Peserta
//      Umum + Nama Referral), "Data Diri Peserta Umum - BINUSIAN", dan "- Non-BINUSIAN".
//      Seluruh field di cabang YAC, ACC, dan Guru Pendamping disalin PERSIS dari config -
//      tidak ada field yang ditambah, dibuang, atau diubah tipe/keterangan/opsinya.
//   3. URUTAN SECTION DIKELOMPOKKAN PER CABANG. Config menyusunnya selang-seling (Tim ACC,
//      Tim YAC, Ketua ACC, Ketua YAC, ...). Karena percabangan hanya mengarahkan ke section
//      PERTAMA tiap cabang dan sisanya mengalir berurutan, susunan selang-seling membuat
//      peserta ACC nyasar ke section YAC setelah section pertamanya. Di sini tiap cabang
//      dibuat kontigu: YAC (Tim -> Ketua -> Anggota 1 -> Anggota 2 -> Kehadiran), lalu ACC
//      dengan pola yang sama, lalu Guru Pendamping, baru section bersama di akhir.
//      >>> CEK JUGA URUTAN INI DI EVENT BERBAYAR - kemungkinan besar masalahnya sama. <<<
//   4. Field "Tipe Peserta (Participant Type)": opsi "Umum" DIGANTI "Guru Pendamping YAC".
//      Opsi "Umum" tidak bisa dipertahankan karena section tujuannya sudah tidak ada di event
//      ini. Ini SATU-SATUNYA field yang isinya berbeda dari config.
//   5. Section "Informasi Kehadiran Peserta Umum" DIGANTI NAMA jadi "Informasi Kehadiran Guru
//      Pendamping YAC" - field di dalamnya sama persis. Di form berbayar guru pendamping masuk
//      kategori Umum, di sini cabang Umum tidak ada sehingga nama lama menyesatkan.
//   6. Step 2 States: referralEnabled OFF (field Nama Referral ada di cabang Umum yang dibuang)
//      dan detailedPricing OFF (event gratis, satu sel harga - sekalian menghindari risiko DOM
//      Time-Price bergeser). Team size tetap ON min 1 max 1 seperti config.
//
...
var SECTIONS = [
    ...
    "fields": [
      {
        ...
        "required": true,
        "description": "Pilih sesuai keikutsertaan Anda di BNAT XV 2026. Event ini KHUSUS peserta lomba (YAC/ACC) dan Guru Pendamping YAC. Peserta Umum (BINUSIAN / Non-BINUSIAN) mendaftar lewat event talkshow berbayar.",
        "options": [
  ...
  {
    "title": "Informasi Kehadiran Guru Pendamping YAC",
    "fields": [
      {
        "label": "Kehadiran Talkshow (Peserta)",
        "type": "multiple_choice",
