// IESCUnpad ISCAAS (National) - Paper Phase & Add 8 Fields (Rp 199.000)
...
// 5. Script fills phase details + adds 8 fields
// 6. Click "Save Timeline" to save
// Note: no Payment Proof field — National payment is handled by our own payment system

var FIELDS = [
  { key: 'Email_paper', label: 'Email', type: 'email', required: true },
  { key: 'TeamName_paper', label: 'Team Name', type: 'text', required: true,
    notes: 'ex. Three Musketeers' },
  { key: 'Institution_paper', label: 'Institution', type: 'text', required: true,
    notes: 'ex. Universitas Padjadjaran' },
  { key: 'ContactPerson_paper', label: 'Contact Person (LINE ID / WhatsApp Number)', type: 'text', required: true,
    notes: 'ex. @anindytavthrni / 082xxxx' },
  { key: 'TeamMember1_paper', label: 'Team Member 1', type: 'text', required: true },
  { key: 'TeamMember2_paper', label: 'Team Member 2 (If Any)', type: 'text', required: false },
  { key: 'TeamMember3_paper', label: 'Team Member 3 (If Any)', type: 'text', required: false },
  { key: 'FullPaper_paper', label: 'Full Paper Submission', type: 'file', required: true }
];
...
(function run() {
  ...
  fill(byName('timelines.' + PHASE_INDEX + '.name'), 'Paper');
  fill(byName('timelines.' + PHASE_INDEX + '.startDate'), '2026-09-19T00:00');
  fill(byName('timelines.' + PHASE_INDEX + '.endDate'), '2026-10-21T23:59');
  fill(byName('timelines.' + PHASE_INDEX + '.price'), '199000');
