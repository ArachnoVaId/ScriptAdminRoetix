// IGNITE WPC - Finals Registration Phase & Add 5 Fields
...
var FIELDS = [
  { key: 'TeamName_finalsregistration', label: 'Team name', type: 'text', required: true },
  { key: 'LeaderFullName_finalsregistration', label: 'Team leader\'s full name', type: 'text', required: true },
  { key: 'LeaderEmail_finalsregistration', label: 'Team leader\'s email', type: 'email', required: true },
  { key: 'LeaderPhoneNumber_finalsregistration', label: 'Team leader\'s active phone number', type: 'phone', required: true },
  { key: 'StatementOfCommitment_finalsregistration', label: 'Statement of Commitment', type: 'file', required: true }
];
...
(function run() {
  ...
  fill(byName('timelines.' + PHASE_INDEX + '.name'), 'Finals Registration');
  ...
  (async function addAllFields() {
    for (var i = 0; i < FIELDS.length; i++) {
      ...
      if (field.type === 'multiple_choice' && field.options && field.options.length > 0) {
        var fieldContainer = keyInput ? keyInput.closest('.space-y-2.rounded-md.border') : null;
        if (fieldContainer) {
          var choiceInput = fieldContainer.querySelector('input[placeholder="Add choice…"]');
          var addChoiceBtn = choiceInput ? choiceInput.parentElement.querySelector('button[type="button"]') : null;
          if (choiceInput && addChoiceBtn) {
            for (var j = 0; j < field.options.length; j++) {
              fill(choiceInput, field.options[j]);
              await sleep(200);
              addChoiceBtn.click();
              console.log('  Choice: ' + field.options[j]);
              await sleep(300);
            }
          }
        }
