// GMBCC Team National - Autofill (paste in browser console on the registration form page)
// Works for: Early Bird / Normal / Extended
(async () => {
...
%%EOF`;
  ...
  fillByName(`TeamName${sf}`, `Team Alpha ${s}`);
  ...
  fillByName(`FullNameAnggota1${sf}`, 'Siti Nurhaliza Putri');
  fillByName(`NationalityAnggota1${sf}`, 'Indonesian');
  fillByName(`UniversityAnggota1${sf}`, 'Universitas Gadjah Mada');
  fillByName(`FacultyAnggota1${sf}`, 'Faculty of Economics and Business');
  fillByName(`MajorAnggota1${sf}`, 'Accounting');
  fillByName(`DomicileAnggota1${sf}`, 'Jakarta');
  fillByName(`PhoneNumberAnggota1${sf}`, '082345678901');
  fillByName(`EmailAnggota1${sf}`, `siti.nurhaliza.${s}@testmail.com`);
  fillByName(`LinkedinAnggota1${sf}`, 'https://linkedin.com/in/sitinurhaliza');
  fillByName(`LinkTwibbonAnggota1${sf}`, 'https://instagram.com/p/example_twibbon_2');
  uploadSectionFiles([`file-CVAnggota1${sf}`, `file-FotoKTMAnggota1${sf}`]);
  uploadAllVisibleFileInputs();
  ...
  fillByName(`FullNameAnggota2${sf}`, 'Budi Santoso Wijaya');
  fillByName(`NationalityAnggota2${sf}`, 'Indonesian');
  fillByName(`UniversityAnggota2${sf}`, 'Universitas Gadjah Mada');
  fillByName(`FacultyAnggota2${sf}`, 'Faculty of Economics and Business');
  fillByName(`MajorAnggota2${sf}`, 'Business Administration');
  fillByName(`DomicileAnggota2${sf}`, 'Surabaya');
  fillByName(`PhoneNumberAnggota2${sf}`, '083456789012');
  fillByName(`EmailAnggota2${sf}`, `budi.santoso.${s}@testmail.com`);
  fillByName(`LinkedinAnggota2${sf}`, 'https://linkedin.com/in/budisantoso');
  fillByName(`LinkTwibbonAnggota2${sf}`, 'https://instagram.com/p/example_twibbon_3');
  uploadSectionFiles([`file-CVAnggota2${sf}`, `file-FotoKTMAnggota2${sf}`]);
  uploadAllVisibleFileInputs();
